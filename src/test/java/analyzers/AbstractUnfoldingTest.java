...
public class AbstractUnfoldingTest {
  ...
  private boolean rulesContainStringRule(List<AbstractUnfoldingAnalyzer.AbstractRule> rules, String left, String right) {
    for (AbstractUnfoldingAnalyzer.AbstractRule r : rules) {
      if (r.isUseful()) {
  ...
  public void testAbstractionTRSUntyped() {
    ...
    var abstractUnfolder = new AbstractUnfoldingAnalyzer(trs);
    List<AbstractUnfoldingAnalyzer.AbstractRule> abstractedRules = abstractUnfolder.abstractionTest(getRulesFromTRS(trs));
  ...
  public void testUnfoldTRSTyped() {
    ...
    TRS trs = createTermRewritingSystem(typedSymbols(), new ArrayList<>(List.of(new FirstOrderRule(r1l, r1r), new FirstOrderRule(r2l, r2r), new FirstOrderRule(r3l, xa), new FirstOrderRule(r4l, xb), new FirstOrderRule(r5l, yb))));
    var abstractUnfolder = new AbstractUnfoldingAnalyzer(trs);
    List<AbstractUnfoldingAnalyzer.AbstractRule> unfoldedRules1 = abstractUnfolder.unfoldTest(getRulesFromTRS(trs));
